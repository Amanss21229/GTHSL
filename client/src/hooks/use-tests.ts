...
export function useSubmitTest() {
  ...
  const submitTest = async (testId: string, answers: Record<number, number>, questions: Question[], timeSpent: number) => {
    try {
      const correctCount = questions.filter(q => answers[q.questionNumber] === q.correctOption).length;
